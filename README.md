...
* `if(조건식){참실행}else{조건식이 거짓일 때 실행결과}`
...
* 배열과 함께 사용하는 경우
### while
* for로 만들어진 반복문을 비교적 간결하게 처리할 수 있는 반복문. 반복 횟수보다는 주어진 조건이 '참'일 경우 반복문을 실행
* 반복 횟수보다는 주어진 조건이 '참'일 경우 반복문을 실행
### 단일 for문 작성법
1. 변수에 초기값을 부여한다.
2. 초기값을 조건식에 적용한다.
3. 참이면 반복문을 실행, 거짓이면 반복문을 종료한다.
4. 반복문 내의 프로그래밍을 실행한다.
5. 초기값을 증가 또는 감소한다. 조건이 참일 경우 3~5번 반복한다.
* `for(초기값; 조건식; 증감식){실행구문;}`
* 예) '자바스크립트' 문자열을 2번 실행시키는 반복문 `for(let i=1; i<=2; i++){console.log('자바스크립트')}`
* 다중 for문 작성법 `for(초기값;조건식;증감식){실행구문1; for(초기값;조건식;증감식){실행구문2;}}`
## ES6 반복문 for~in, for~of
### for(변수생성 in 접근배열명)
* `for(let i in 접근배열명){실행}`
* 배열 인덱스를 출력
### for(변수생성 of 접근배열명)
* `for(let i of 접근배열명){실행}`
* 배열 객체를 출력